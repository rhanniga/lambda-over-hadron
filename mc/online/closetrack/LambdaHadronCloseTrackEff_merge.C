void LambdaHadronCloseTrackEff_merge(const char *dir, Int_t stage=0)
{
...
// Analysis source to be compiled at runtime (if any)
   gROOT->ProcessLine(".L AliAnalysisTaskLambdaHadronCloseTrackEff.cxx+g");
...
// Set temporary compilation directory to current one
   ...
   TString outputFile;
   Bool_t merged = kTRUE;
   AliAnalysisManager *mgr = AliAnalysisAlien::LoadAnalysisManager("LambdaHadronCloseTrackEff.root");
   if (!mgr) {
   ...
   while((str=(TObjString*)iter->Next())) {
      ...
      merged = AliAnalysisAlien::MergeOutput(outputFile, outputDir, 15, stage);
      if (!merged) {
