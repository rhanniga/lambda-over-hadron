...
for pt_bin in ["central", "low", "high"]:
    for cent_bin in [0, 20, 50, 80]:
        ...
        if cent_bin == 0:
            ...
            lambda_mass_dist = v0_input_file.Get("lambda_mass_dist_0_80").Clone("tmp")
            lambda_mass_fit = v0_input_file.Get("lambda_mass_fit_0_80")
            lambda_mass_bg_fit = v0_input_file.Get("bg_fit_0_80")
        else:
        ...
        signal_region_low = 1.102
        signal_region_high = 1.130
        lambda_signal_region_box = rt.TBox(signal_region_low, 0, signal_region_high, lambda_mass_dist.GetMaximum())
        lambda_signal_region_box.SetFillColorAlpha(rt.kGreen + 2, 0.3)
        lambda_signal_region_box.SetFillStyle(1001)
        lambda_signal_region_box.SetLineColor(rt.kGreen + 2)
        lambda_signal_region_box.SetLineWidth(2)

        sideband_region_low = 1.135
        sideband_region_high = 1.150
        lambda_sideband_region_box = rt.TBox(sideband_region_low, 0, sideband_region_high, lambda_mass_dist.GetMaximum())
        lambda_sideband_region_box.SetFillColorAlpha(rt.kOrange + 2, 0.3)
        lambda_sideband_region_box.SetFillStyle(1001)
        lambda_sideband_region_box.SetLineColor(rt.kOrange + 2)
        lambda_sideband_region_box.SetLineWidth(2)

        fit_integral = lambda_mass_fit.Integral(signal_region_low, signal_region_high)
        bg_integral = lambda_mass_bg_fit.Integral(signal_region_low, signal_region_high)
        signal_integral = fit_integral - bg_integral

        print(f"Centrality bin: {cent_bin}")
        print(f"Momentum bin: {pt_bin}")
        print(f"Signal integral: {signal_integral/0.001}")

        lambda_mass_dist.Draw()
        lambda_mass_fit.Draw("SAME")
        lambda_mass_bg_fit.Draw("SAME")
        lambda_signal_region_box.Draw("SAME")
        lambda_sideband_region_box.Draw("SAME")
        ...
        region_legend = rt.TLegend(legend_x_start + 0.15, legend_y_start, legend_x_finish + 0.15, legend_y_finish)
        region_legend.SetTextSize(0.035)
        region_legend.SetBorderSize(0)
        region_legend.SetFillStyle(0)
        region_legend.AddEntry(lambda_signal_region_box, "Sig. reg.", "f")
        region_legend.AddEntry(lambda_sideband_region_box, "SB reg.", "f")
        region_legend.Draw("SAME")
        ...
        if cent_bin == 0:
            ...
            alice_data_label.DrawLatex(label_x_start, label_y_start - 2*label_text_space, "#bf{0#minus80% V0A}")
        ...
        if pt_bin == "central":
            if cent_bin == 0:
                ...
                c.SaveAs("figures/lambda_mass_dist_0_80.eps")
        elif pt_bin == "low":
            if cent_bin == 0:
                ...
                c.SaveAs("figures/lambda_mass_dist_0_80_lowpt.eps")
        elif pt_bin == "high":
            if cent_bin == 0:
                ...
                c.SaveAs("figures/lambda_mass_dist_0_80_highpt.eps")
        ...
        if pt_bin == "central":
            if cent_bin == 0:
                ...
                c.SaveAs("figures/lambda_mass_dist_0_80.pdf")
        elif pt_bin == "low":
            if cent_bin == 0:
                ...
                c.SaveAs("figures/lambda_mass_dist_0_80_lowpt.pdf")
        elif pt_bin == "high":
            if cent_bin == 0:
                ...
                c.SaveAs("figures/lambda_mass_dist_0_80_highpt.pdf")
