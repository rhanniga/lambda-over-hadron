...
for key in h_lambda_near_dict:
    near_central_value = 0
    near_central_error = 0
    away_central_value = 0
    away_central_error = 0
    ue_central_value = 0
    ue_central_error = 0
    total_central_value = 0
    total_central_error = 0
    n_list = []
    if key == CENTRAL_TECHNIQUE:
        near_central_value = h_lambda_near_dict[key].GetY()[2]
        near_central_error = h_lambda_near_dict[key].GetEY()[2]

        away_central_value = h_lambda_away_dict[key].GetY()[2]
        away_central_error = h_lambda_away_dict[key].GetEY()[2]

        ue_central_value = h_lambda_ue_dict[key].GetY()[2]
        ue_central_error = h_lambda_ue_dict[key].GetEY()[2]

        total_central_value = h_lambda_total_dict[key].GetY()[2]
        total_central_error = h_lambda_total_dict[key].GetEY()[2]

    else:
        delta_y1 = h_lambda_near_dict[key].GetY()[2] - near_central_value
        delta_e1 = math.sqrt(rt.TMath.Abs(h_lambda_near_dict[key].GetEY()[2]**2 - near_central_error**2))
        delta_y2 = h_lambda_away_dict[key].GetY()[2] - away_central_value
        delta_e2 = math.sqrt(rt.TMath.Abs(h_lambda_away_dict[key].GetEY()[2]**2 - away_central_error**2))
        delta_y3 = h_lambda_ue_dict[key].GetY()[2] - ue_central_value
        delta_e3 = math.sqrt(rt.TMath.Abs(h_lambda_ue_dict[key].GetEY()[2]**2 - ue_central_error**2))
        delta_y4 = h_lambda_total_dict[key].GetY()[2] - total_central_value
        delta_e4 = math.sqrt(rt.TMath.Abs(h_lambda_total_dict[key].GetEY()[2]**2 - total_central_error**2))
        print(f"--------------{key}--------------")
        print(delta_y1/delta_e1, " = near")
        print(delta_y2/delta_e2, " = away")
        print(delta_y3/delta_e3, " = ue")
        print(delta_y4/delta_e4, " = total")
